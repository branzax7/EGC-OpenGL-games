...
void Tema2::Init()
{
...

    {
        ...
        //camera2->position = glm::vec3(carX, carY + 0.5, carZ + carView);
...
void Tema2::RenderScene()
{
    ...
    for (glm::vec3 x : points) {
        ...
        RenderMesh(meshes["trunchi"], shaders["LabShader"], modelMatrix); //VertexColor
        ...
        RenderMesh(meshes["frunze"], shaders["LabShader"], modelMatrix);
        modelMatrix = glm::mat4(1);
    ...
    RenderMesh(meshes["iarba"], shaders["LabShader"], modelMatrix);
    
    modelMatrix = glm::mat4(1);
    modelMatrix = glm::scale(modelMatrix, glm::vec3(3, 3, 3));
    RenderMesh(meshes["pista"], shaders["LabShader"],modelMatrix);
    ...
    if (renderCameraTarget)
    {
        ...
        modelMatrix = glm::scale(modelMatrix, glm::vec3(1, 1, 2));
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-carSize / 2, 0, -carSize / 2));
        RenderMesh(meshes["masina"], shaders["VertexShader"], modelMatrix);
    }
}
...
void Tema2::Update(float deltaTimeSeconds) {
    ...
    cout << "Camera2: " << camera2->position << endl;
    cout << "Camera1: " << camera->position << endl;
