...
void main()
{
    ...
    float specular_light = material_ks * 1 * pow(max(dot(N, H), 0), material_shininess);
    ...
    flaot dist = distance (light_position, world_pos);
    flaot att_factor = 1.0 / pow((dist + 1), 2);

    // TODO(student): Send color light output to fragment shader
    color = object_color * (ambient_light + Ke + diffuse_light + specular_light);
